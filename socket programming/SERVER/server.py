...
def list_files():
    ...
    for i in listing:
        ...
        # File name
        conn.send(bytes(i, encoding='ascii'))
        # File content size
...
def user_authentication():
    ...
    conn.send(b"1")
    # Recieve user name
    user_name_size = struct.unpack("h", conn.recv(2))[0]
    user_name = conn.recv(user_name_size)
    # Send message to let client know server is ready for document content
    conn.send(b"1")
    #Check if user name exsits
    num=0
    if(user_name==(b"RAMITH") or user_name==(b"PRANAV") or user_name==(b"KISHAN")):
        num=1
